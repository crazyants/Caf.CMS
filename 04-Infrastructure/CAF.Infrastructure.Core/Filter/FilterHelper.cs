...
namespace CAF.Infrastructure.Core.Filter
{
    ...
    public static class FilterHelper
    {
        ...
        public static string ToOperateCode(this FilterOperate operate)
        {
            ...
            MemberInfo[] members = type.GetMember(operate.Convert<string>());
            if (members.Length > 0)
            {
        ...
        public static FilterOperate GetFilterOperate(string code)
        {
            ...
            {
                FilterOperate operate = member.Name.Convert<FilterOperate>();
        ...
        private static LambdaExpression GetPropertyLambdaExpression(ParameterExpression param, FilterRule rule)
        {
            ...
            foreach (string propertyName in propertyNames)
            {
                ...
                if (property == null)
                {
                    throw new WorkException(string.Format("指定的属性“{0}”在类型“{1}”中不存在。 的本地化字符串" ,rule.Field, type.FullName));
                }
        ...
        private static Expression ChangeTypeToExpression(FilterRule rule, Type conversionType)
        {
            ...
            Type elementType = conversionType.GetNonNullableType();
            object value = rule.Value is string
                ? rule.Value.ToString().Convert(conversionType)
