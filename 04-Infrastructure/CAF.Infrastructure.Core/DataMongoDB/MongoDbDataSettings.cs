...
using CAF.Infrastructure.Core.Utilities.Threading;
using CAF.Infrastructure.Core.Utilities;

namespace CAF.Infrastructure.Core.Data
{

    public partial class MongoDbDataSettings
    {
		private static readonly ReaderWriterLockSlim s_rwLock = new ReaderWriterLockSlim();
		private static MongoDbDataSettings s_current = null;
		private static Func<MongoDbDataSettings> s_settingsFactory = new Func<MongoDbDataSettings>(() => new MongoDbDataSettings());
		private static bool? s_installed = null;
		private static bool s_TestMode = false;

		protected const char SEPARATOR = ':';
		protected const string FILENAME = "MongoDbSettings.txt";

		private MongoDbDataSettings()
        {
            RawMongoDbDataSettings = new Dictionary<string, string>();
        }

		#region Static members

		public static void SetDefaultFactory(Func<MongoDbDataSettings> factory) 
		{
		...
		public static MongoDbDataSettings Current
		{
			get
			{
        ...
        public IDictionary<string, string> RawMongoDbDataSettings 
		{ 
			get; 
			private set; 
		}
		...
		public virtual bool Load()
		{
			using (s_rwLock.GetWriteLock())
			{
				...
				if (File.Exists(filePath))
				{
					...
					if (settings.Any())
					{
						this.RawMongoDbDataSettings.AddRange(settings);
		...
		public void Reset()
		{
			using (s_rwLock.GetWriteLock())
			{
				this.RawMongoDbDataSettings.Clear();
