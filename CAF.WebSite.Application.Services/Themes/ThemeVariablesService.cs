...
namespace CAF.WebSite.Application.Services.Themes
{

    public class ThemeVariablesService : IThemeVariablesService
    {
        private const string THEMEVARS_BY_THEME_KEY = "CafSite.themevars.theme-{0}-{1}";
        private const string THEMEVARS_PATTERN_KEY = "CafSite.themevars.";
        ...
        public virtual ExpandoObject GetThemeVariables(string themeName, int storeId)
        {
            ...
            return _cacheManager.Get(key, () =>
            {
                ...
                var query = from v in _rsVariables.Table
                            where v.SiteId == storeId && v.Theme.Equals(themeName, StringComparison.OrdinalIgnoreCase)
                            select v;
                query.Each(v =>
                {
        ...
        public virtual int SaveThemeVariables(string themeName, int storeId, IDictionary<string, object> variables)
        {
            ...
            using (var scope = new DbContextScope(ctx: _rsVariables.Context, autoCommit: false))
            {
                ...
                var savedThemeVars = _rsVariables.Table.Where(v => v.SiteId == storeId && v.Theme.Equals(themeName, StringComparison.OrdinalIgnoreCase)).ToList();
                bool touched = false;
        ...
        public int ImportVariables(string themeName, int storeId, string configurationXml)
        {
            ...
            {
                throw new WorkException("The theme reference in the import file ('{0}') does not match the current theme '{1}'.".FormatCurrent(forTheme.ToSafe(), themeName));
            }
