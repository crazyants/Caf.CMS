...
using CAF.Infrastructure.Core.ImageLibrary;
...
namespace CAF.WebSite.Application.Services.Media
{
    ...
    public partial class PictureService : IPictureService
    {
                    ...
                    if (!File.Exists(cachedImage.LocalPath)) // check again
                    {
                        ...
                        try
                        {
                            if (targetSize == 0)
                            {
                                ...
                                //此处添加水印等处理
                                AddImageToWatermark(cachedImage.LocalPath);
                            }
                            else
                            {
        ...
        public virtual IList<Picture> GetPicturesByArticleId(int articleId, int recordsToReturn = 0)
        {
            ...
            var query = from p in _pictureRepository.Table
                        join pp in _articleAlbumRepository.Table on p.Id equals pp.PictureId
