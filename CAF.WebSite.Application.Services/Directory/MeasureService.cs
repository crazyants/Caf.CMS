...
namespace CAF.WebSite.Application.Services.Directory
{
    ...
    {
        #region Constants
        private const string MEASUREDIMENSIONS_ALL_KEY = "cafsite.measuredimension.all";
        private const string MEASUREWEIGHTS_ALL_KEY = "cafsite.measureweight.all";
        private const string MEASUREDIMENSIONS_PATTERN_KEY = "cafsite.measuredimension.";
        private const string MEASUREWEIGHTS_PATTERN_KEY = "cafsite.measureweight.";
        private const string MEASUREDIMENSIONS_BY_ID_KEY = "cafsite.measuredimension.id-{0}";
        private const string MEASUREWEIGHTS_BY_ID_KEY = "cafsite.measureweight.id-{0}";
        #endregion
            ...
            MeasureDimension sourceMeasureDimension)
        {
            ...
            if (result != decimal.Zero && sourceMeasureDimension.Id != baseDimensionIn.Id)
            {
                ...
                if (exchangeRatio == decimal.Zero)
                    throw new WorkException(string.Format("Exchange ratio not set for dimension [{0}]", sourceMeasureDimension.Name));
            ...
            MeasureDimension targetMeasureDimension)
        {
            ...
            if (result != decimal.Zero && targetMeasureDimension.Id != baseDimensionIn.Id)
            {
                ...
                if (exchangeRatio == decimal.Zero)
                    throw new WorkException(string.Format("Exchange ratio not set for dimension [{0}]", targetMeasureDimension.Name));
                result = result * exchangeRatio;
            }
            return result;
        }
        ...
        public virtual decimal ConvertToPrimaryMeasureWeight(decimal quantity, MeasureWeight sourceMeasureWeight)
        {
            ...
            if (result != decimal.Zero && sourceMeasureWeight.Id != baseWeightIn.Id)
            {
                ...
                if (exchangeRatio == decimal.Zero)
                    throw new WorkException(string.Format("Exchange ratio not set for weight [{0}]", sourceMeasureWeight.Name));
                result = result / exchangeRatio;
            }
            return result;
        }
            ...
            MeasureWeight targetMeasureWeight)
        {
            ...
            if (result != decimal.Zero && targetMeasureWeight.Id != baseWeightIn.Id)
            {
                ...
                if (exchangeRatio == decimal.Zero)
                    throw new WorkException(string.Format("Exchange ratio not set for weight [{0}]", targetMeasureWeight.Name));
                result = result * exchangeRatio;
            }
            return result;
        }
