...
namespace CAF.WebSite.Application.Services.Forums
{
    /// <summary>
    /// Forum service
    /// </summary>
    ...
    {
        #region Constants
        private const string FORUMGROUP_ALL_KEY = "CafSite.forumgroup.all";
        private const string FORUM_ALLBYFORUMGROUPID_KEY = "CafSite.forum.allbyforumgroupid-{0}";
        private const string FORUMGROUP_PATTERN_KEY = "CafSite.forumgroup.";
        private const string FORUM_PATTERN_KEY = "CafSite.forum.";
        #endregion
        ...
        /// </summary>
		/// <param name="siteId">The store identifier; pass 0 to load all messages</param>
		...
		public virtual IPagedList<PrivateMessage> GetAllPrivateMessages(int siteId, int fromUserId,
            ...
            string keywords, int pageIndex, int pageSize)
        {
			...
			if (siteId > 0)
				query = query.Where(pm => siteId == pm.SiteId);
            if (fromUserId > 0)
        ...
        public virtual void InsertPrivateMessage(PrivateMessage privateMessage)
        {
            ...
            if (userTo == null)
            {
                throw new WorkException("Recipient could not be loaded");
            }

			//UI notification
			_genericAttributeService.SaveAttribute(userTo, SystemUserAttributeNames.NotifiedAboutNewPrivateMessages, false, privateMessage.SiteId);

            //Email notification
            if (_forumSettings.NotifyAboutPrivateMessages)
            {
                _workflowMessageService.SendPrivateMessageNotification(userTo, privateMessage, _workContext.WorkingLanguage.Id);                
            }
        }
