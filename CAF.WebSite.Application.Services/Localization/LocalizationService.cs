...
namespace CAF.WebSite.Application.Services.Localization
{
    ...
    {
        #region Constants
        private const string LOCALESTRINGRESOURCES_ALL_KEY = "CrAnt.FrameWork.lsr.all-{0}";
        private const string LOCALESTRINGRESOURCES_PATTERN_KEY = "CrAnt.FrameWork.lsr.";
        #endregion
        ...
        public virtual IList<LocaleStringResource> GetAllResources(int languageId, string resourceName = null)
        {
            ...
            if (!resourceName.IsEmpty())
                query = query.Where(r => r.ResourceName.Contains(resourceName));
        ...
        public virtual IDictionary<string, Tuple<int, string>> GetResourceValues(int languageId, string rName = null, bool forceAll = false)
        {
            Action<ConcurrentDictionary<string, Tuple<int, string>>> loadAllAction = (d) =>
            {
                var locales = this.GetAllResources(languageId, rName);
            ...
            if (forceAll && !dict.ContainsKey("!!___EOF___!!"))
            {
        ...
        private class LocaleStringResourceParent : LocaleStringResource
        {
            public LocaleStringResourceParent(XmlNode localStringResource, string nameSpace = "")
            {
                ...
                {
                    throw new WorkException("All language resources must have an attribute Name=\"Value\".");
                }
                var resName = resNameAttribute.Value.Trim();
                if (string.IsNullOrEmpty(resName))
                {
                    throw new WorkException("All languages resource attributes 'Name' must have a value.'");
                }
