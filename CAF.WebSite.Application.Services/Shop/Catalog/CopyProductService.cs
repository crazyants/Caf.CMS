    ...
    public partial class CopyProductService : ICopyProductService
    {
		...
		public virtual Product CopyProduct(Product product, string newName, bool isPublished, bool copyImages, bool copyAssociatedProducts = true)
        {
            ...
            productCopy = new Product()
            {
				...
				LimitedToSites = product.LimitedToSites,
				Sku = product.Sku,
			...
			//store mapping
			var selectedSiteIds = _siteMappingService.GetSitesIdsWithAccess(product);
			foreach (var id in selectedSiteIds)
			{
				_siteMappingService.InsertSiteMapping(productCopy, id);
			}
