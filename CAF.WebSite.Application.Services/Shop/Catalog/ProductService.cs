...
namespace CAF.WebSite.Application.Services.Catalog
{
    ...
    public partial class ProductService : IProductService
	{
		...
		private const string PRODUCTS_BY_ID_KEY = "WebSite.product.id-{0}";
		private const string PRODUCTS_PATTERN_KEY = "WebSite.product.";
        ...
        public virtual IPagedList<Product> SearchProducts(ProductSearchContext ctx)
        {
            ...
            else
            {
                ...
                var query = this.PrepareProductSearchQuery(ctx, allowedUserRolesIds, searchLocalizedValue);
		...
		public virtual IQueryable<Product> PrepareProductSearchQuery(
			...
			bool searchLocalizedValue = false)
		{
			return PrepareProductSearchQuery<Product>(ctx, x => x, allowedUserRolesIds, searchLocalizedValue);
		}
			...
			bool searchLocalizedValue = false)
		{
			...
			if (ctx.SiteId > 0 && !QuerySettings.IgnoreMultiSite)
			{
				query =
					from p in query
					join sm in _siteMappingRepository.Table on new { pid = p.Id, pname = "Product" } equals new { pid = sm.EntityId, pname = sm.EntityName } into psm
                ...
                case ManageInventoryMethod.ManageStock:
                    {
						...
						if (decrease && product.NotifyAdminForQuantityBelow > result.StockQuantityNew)
                            _workflowMessageService.SendQuantityBelowSiteOwnerNotification(product, _localizationSettings.DefaultAdminLanguageId);                        
                    }
                    break;
		...
		public virtual void InsertBundleItem(ProductBundleItem bundleItem)
		{
			...
			if (bundleItem.BundleProductId == 0)
				throw new WorkException("BundleProductId of a bundle item cannot be 0.");

			if (bundleItem.ProductId == 0)
				throw new WorkException("ProductId of a bundle item cannot be 0.");

			if (bundleItem.ProductId == bundleItem.BundleProductId)
				throw new WorkException("A bundle item cannot be an element of itself.");
