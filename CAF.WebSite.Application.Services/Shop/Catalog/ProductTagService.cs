...
namespace CAF.WebSite.Application.Services.Catalog
{
    ...
    public partial class ProductTagService : IProductTagService
    {
		...
		/// {0} : store ID
		/// </remarks>
		private const string PRODUCTTAG_COUNT_KEY = "caf.producttag.count-{0}";

		/// <summary>
		/// Key pattern to clear cache
		/// </summary>
		private const string PRODUCTTAG_PATTERN_KEY = "caf.producttag.";
		...
		private Dictionary<int, int> GetProductCount(int siteId)
		{
			string key = string.Format(PRODUCTTAG_COUNT_KEY, siteId);
		...
		public virtual int GetProductCount(int productTagId, int siteId)
		{
			var dictionary = GetProductCount(siteId);
