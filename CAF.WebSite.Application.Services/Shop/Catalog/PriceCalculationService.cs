...
namespace CAF.WebSite.Application.Services.Catalog
{
    ...
    public partial class PriceCalculationService : IPriceCalculationService
    {
        ...
        protected virtual decimal? GetMinimumTierPrice(Product product, User user, int quantity)
        {
            ...
            var tierPrices = product.TierPrices
				...
				.FilterBySite(_commonServices.SiteContext.CurrentSite.Id)
                .FilterForUser(user)
                .ToList()
        ...
        {
            var user = _commonServices.WorkContext.CurrentUser;
			return GetFinalPrice(product, user, includeDiscounts);
        }
		...
		public virtual decimal GetFinalPrice(Product product, 
            ...
            bool includeDiscounts)
        {
			return GetFinalPrice(product, user, decimal.Zero, includeDiscounts);
        }
		...
		public virtual decimal GetFinalPrice(Product product, 
            ...
            bool includeDiscounts)
        {
            return GetFinalPrice(product, user, additionalCharge, includeDiscounts, 1);
        }
			...
			ProductBundleItemData bundleItem = null)
        {
            ...
            {
				decimal? tierPrice = GetMinimumTierPrice(product, user, quantity);
            ...
            if (includeDiscounts)
            {
				...
				decimal discountAmount = GetDiscountAmount(product, user, additionalCharge, quantity, out appliedDiscount, bundleItem);
			...
			ProductBundleItemData bundleItem = null)
		{
			if (product.ProductType == ProductType.BundledProduct && product.BundlePerItemPricing)
			{
				...
				{
					decimal itemPrice = GetFinalPrice(itemData.Item.Product, user, itemData.AdditionalCharge, includeDiscounts, 1, itemData);
			...
			return GetFinalPrice(product, user, additionalCharge, includeDiscounts, quantity, bundleItem);
		}
		...
		public virtual decimal GetLowestPrice(Product product, out bool displayFromMessage)
		{
			...
			decimal lowestPrice = GetFinalPrice(product, bundleItems, _commonServices.WorkContext.CurrentUser, decimal.Zero, true, int.MaxValue);
		...
		public virtual decimal? GetLowestPrice(Product product, IEnumerable<Product> associatedProducts, out Product lowestPriceProduct)
		{
			...
			{
				var tmpPrice = GetFinalPrice(associatedProduct, _commonServices.WorkContext.CurrentUser, decimal.Zero, true, int.MaxValue);
        ...
        {
            var user = _commonServices.WorkContext.CurrentUser;
            return GetDiscountAmount(product, user, decimal.Zero);
        }
        ...
        {
            Discount appliedDiscount = null;
            return GetDiscountAmount(product, user, additionalCharge, out appliedDiscount);
        }
        ...
        {
            return GetDiscountAmount(product, user, additionalCharge, 1, out appliedDiscount);
        }
			...
			ProductBundleItemData bundleItem = null)
        {
			...
			if (bundleItem.IsValid())
			{
				if (bundleItem.Item.Discount.HasValue && bundleItem.Item.BundleProduct.BundlePerItemPricing)
				{
					...
					finalPriceWithoutDiscount = GetFinalPrice(product, user, additionalCharge, false, quantity, bundleItem);
					appliedDiscountAmount = appliedDiscount.GetDiscountAmount(finalPriceWithoutDiscount);
				}
			}
			else
			{
				// dont't apply when user entered price or discounts should be ignored completely
				if (product.UserEntersPrice || _catalogSettings.IgnoreDiscounts)
				{
		...
		public virtual decimal GetUnitPrice(OrganizedShoppingCartItem shoppingCartItem, bool includeDiscounts)
        {
            ...
            var user = shoppingCartItem.Item.User;
            ...
            if (product != null)
            {
                ...
                {
                    finalPrice = shoppingCartItem.Item.UserEnteredPrice;
                }
				else if (product.ProductType == ProductType.BundledProduct && product.BundlePerItemPricing)
				{
					if (shoppingCartItem.ChildItems != null)
					{
						...
						finalPrice = GetFinalPrice(product, bundleItems, user, decimal.Zero, includeDiscounts, shoppingCartItem.Item.Quantity);
					}
				}
                else
                {
					...
					finalPrice = GetFinalPrice(product, user, attributesTotalPrice, includeDiscounts, shoppingCartItem.Item.Quantity, shoppingCartItem.BundleItemData);
                }
            }
		...
		public virtual decimal GetDiscountAmount(OrganizedShoppingCartItem shoppingCartItem, out Discount appliedDiscount)
        {
			...
			if (product != null)
            {
				...
				decimal productDiscountAmount = GetDiscountAmount(product, user, attributesTotalPrice, shoppingCartItem.Item.Quantity, out appliedDiscount);
