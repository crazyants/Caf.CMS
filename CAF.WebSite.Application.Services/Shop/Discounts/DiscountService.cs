...
namespace CAF.WebSite.Application.Services.Discounts
{
    ...
    {
        #region Constants
        private const string DISCOUNTS_ALL_KEY = "WebSite.discount.all-{0}-{1}";
        private const string DISCOUNTS_PATTERN_KEY = "WebSite.discount.";
        #endregion
        ...
        /// <summary>
        /// Checks discount limitation for user
        /// </summary>
        /// <param name="discount">Discount</param>
        /// <param name="user">User</param>
        /// <returns>Value indicating whether discount can be used</returns>
        protected virtual bool CheckDiscountLimitations(Discount discount, User user)
        {
            if (discount == null)
                ...
                case DiscountLimitationType.NTimesPerUser:
                    {
                        ...
                        {
                            //registered user
                            var totalDuh = GetAllDiscountUsageHistory(discount.Id, user.Id, 0, 1).TotalCount;
                            return totalDuh < discount.LimitationTimes;
                        }
                        else
                        {
		...
		public virtual Provider<IDiscountRequirementRule> LoadDiscountRequirementRuleBySystemName(string systemName, int siteId = 0)
        {
			return _providerManager.GetProvider<IDiscountRequirementRule>(systemName, siteId);
        }
		...
		public virtual IEnumerable<Provider<IDiscountRequirementRule>> LoadAllDiscountRequirementRules(int siteId = 0)
        {
			return _providerManager.GetAllProviders<IDiscountRequirementRule>(siteId);
        }
        ...
        public virtual bool IsDiscountValid(Discount discount, User user)
        {
            if (discount == null)
                throw new ArgumentNullException("discount");
            ...
            if (user != null)
				couponCodeToValidate = user.GetAttribute<string>(SystemUserAttributeNames.DiscountCouponCode, _genericAttributeService);

            return IsDiscountValid(discount, user, couponCodeToValidate);
        }
        ...
        /// <returns>true - requirement is met; otherwise, false</returns>
        public virtual bool IsDiscountValid(Discount discount, User user, string couponCodeToValidate)
        {
            if (discount == null)
                throw new ArgumentNullException("discount");
            ...
            if (!CheckDiscountLimitations(discount, user))
                return false;
            ...
            {
				var requirementRule = LoadDiscountRequirementRuleBySystemName(req.DiscountRequirementRuleSystemName, siteId);
			...
			// better not to apply discounts if there are gift cards in the cart cause the user could "earn" money through that.
            ...
            int? userId, int pageIndex, int pageSize)
        {
            ...
            if (userId.HasValue && userId.Value > 0)
                query = query.Where(duh => duh.Order != null && duh.Order.UserId == userId.Value);
