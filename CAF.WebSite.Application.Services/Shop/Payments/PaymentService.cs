...
namespace CAF.WebSite.Application.Services.Payments
{
    ...
    public partial class PaymentService : IPaymentService
    {
		...
		public virtual IEnumerable<Provider<IPaymentMethod>> LoadActivePaymentMethods(int? filterByUserId = null, int siteId = 0)
        {
			var allMethods = LoadAllPaymentMethods(siteId);
		...
		/// </summary>
		public virtual bool IsPaymentMethodActive(string systemName, int siteId = 0)
		{
			var method = LoadPaymentMethodBySystemName(systemName, true, siteId);
			return method != null;
		}
		...
		public virtual Provider<IPaymentMethod> LoadPaymentMethodBySystemName(string systemName, bool onlyWhenActive = false, int siteId = 0)
        {
			var provider = _providerManager.GetProvider<IPaymentMethod>(systemName, siteId);
		...
		public virtual IEnumerable<Provider<IPaymentMethod>> LoadAllPaymentMethods(int siteId = 0)
        {
			return _providerManager.GetAllProviders<IPaymentMethod>(siteId);
        }
		...
		/// Called after an order has been placed or when user re-post the payment.
        /// </summary>
