...
namespace CAF.WebSite.Application.Services.Orders
{
    ...
    public partial class OrderReportService : IOrderReportService
    {
		...
		public virtual OrderAverageReportLine GetOrderAverageReportLine(int siteId, int[] orderStatusIds,
        ...
        /// <returns>Result</returns>
		public virtual OrderAverageReportLineSummary OrderAverageReport(int siteId, OrderStatus os)
        {
            ...
            if (!timeZone.IsInvalidTime(t1))
            {
				...
				var todayResult = GetOrderAverageReportLine(siteId, orderStatusId, null, null, startTime1, endTime1, null);
            ...
            if (!timeZone.IsInvalidTime(t2))
            {
				...
				var weekResult = GetOrderAverageReportLine(siteId, orderStatusId, null, null, startTime2, endTime2, null);
            ...
            if (!timeZone.IsInvalidTime(t3))
            {
				...
				var monthResult = GetOrderAverageReportLine(siteId, orderStatusId, null, null, startTime3, endTime3, null);
            ...
            if (!timeZone.IsInvalidTime(t4))
            {
				...
				var yearResult = GetOrderAverageReportLine(siteId, orderStatusId, null, null, startTime4, endTime4, null);
			...
			var allTimeResult = GetOrderAverageReportLine(siteId, orderStatusId, null, null, startTime5, endTime5, null);
        ...
        /// <returns>Result</returns>
		public virtual IList<BestsellersReportLine> BestSellersReport(int siteId, 
            ...
            int recordsToReturn = 5, int orderBy = 1, bool showHidden = false)
        {
            ...
            var query1 = from orderItem in _orderItemRepository.Table
						 ...
						 where (siteId == 0 || siteId == o.SiteId) &&
		...
		public virtual int[] GetAlsoPurchasedProductsIds(int siteId, int productId, int recordsToReturn = 5, bool showHidden = false)
        {
            if (productId == 0)
            ...
            var query2 = from orderItem in _orderItemRepository.Table
						 ...
						 (siteId == 0 || orderItem.Order.SiteId == siteId) &&
                         (!p.Deleted) &&
                         (showHidden || p.Published)
        ...
        /// <param name="billingEmail">Billing email. Leave empty to load all records.</param>
        /// <returns>Result</returns>
		public virtual decimal ProfitReport(int siteId, int[] orderStatusIds, int[] paymentStatusIds, int[] shippingStatusIds,
			DateTime? startTimeUtc, DateTime? endTimeUtc, string billingEmail)
        {
			...
			var reportSummary = GetOrderAverageReportLine(siteId, orderStatusIds, paymentStatusIds, shippingStatusIds, startTimeUtc, endTimeUtc, billingEmail);
