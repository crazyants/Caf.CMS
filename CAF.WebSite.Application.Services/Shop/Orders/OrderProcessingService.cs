...
using CAF.WebSite.Domain.Seedwork.Logging;
...
namespace CAF.WebSite.Application.Services.Orders
{
    ...
    public partial class OrderProcessingService : IOrderProcessingService
    {
		...
		protected void AwardRewardPoints(Order order, decimal? amount = null)
        {
            ...
            //add reward points
            order.User.AddRewardPointsHistoryEntry(points, string.Format(_localizationService.GetResource("RewardPoints.Message.EarnedForOrder"), order.GetOrderNumber()));
        ...
        protected void ReduceRewardPoints(Order order, decimal? amount = null)
        {
            ...
            //reduce reward points
            order.User.AddRewardPointsHistoryEntry(-points, string.Format(_localizationService.GetResource("RewardPoints.Message.ReducedForOrder"), order.GetOrderNumber()));
                ...
                if (activate)
                {
                    ...
                    if (gc.GiftCardType == GiftCardType.Virtual)
                    {
                            ...
                            !String.IsNullOrEmpty(gc.SenderEmail))
                        {
                            ...
                            int queuedEmailId = _workflowMessageService.SendGiftCardNotification(gc, userLang.Id);
                            if (queuedEmailId > 0)
        ...
        protected void SetOrderStatus(Order order, OrderStatus os, bool notifyUser)
        {
            if (order == null)
            ...
            {
                //notification
                int orderCompletedUserNotificationQueuedEmailId = _workflowMessageService.SendOrderCompletedUserNotification(order, order.UserLanguageId);
                if (orderCompletedUserNotificationQueuedEmailId > 0)
                {
                    order.OrderNotes.Add(new OrderNote()
                    {
                        Note = string.Format(T("UserCompletedEmailQueued"), orderCompletedUserNotificationQueuedEmailId),
                        DisplayToUser = false,
                        CreatedOnUtc = DateTime.UtcNow
                    });
                    _orderService.UpdateOrder(order);
            ...
            {
                //notification
                int orderCancelledUserNotificationQueuedEmailId = _workflowMessageService.SendOrderCancelledUserNotification(order, order.UserLanguageId);
                if (orderCancelledUserNotificationQueuedEmailId > 0)
                {
                    order.OrderNotes.Add(new OrderNote()
                    {
                        Note = string.Format(T("UserCancelledEmailQueued"), orderCancelledUserNotificationQueuedEmailId),
                        DisplayToUser = false,
                        CreatedOnUtc = DateTime.UtcNow
                    });
                    _orderService.UpdateOrder(order);
        ...
        public virtual PlaceOrderResult PlaceOrder(ProcessPaymentRequest processPaymentRequest, Dictionary<string, string> extraData)
        {
            ...
            try
            {
                #region Order details (user, totals)
                ...
                decimal userCurrencyRate = decimal.Zero;
                if (!processPaymentRequest.IsRecurringPayment)
                {
					var currencyTmp = _currencyService.GetCurrencyById(user.GetAttribute<int>(SystemUserAttributeNames.CurrencyId, processPaymentRequest.SiteId));
					var userCurrency = (currencyTmp != null && currencyTmp.Published) ? currencyTmp : _workContext.WorkingCurrency;
                    userCurrencyCode = userCurrency.CurrencyCode;
                    var primarySiteCurrency = _currencyService.GetCurrencyById(_currencySettings.PrimarySiteCurrencyId);
                    userCurrencyRate = userCurrency.Rate / primarySiteCurrency.Rate;
                }
                else
                {
                    userCurrencyCode = initialOrder.UserCurrencyCode;
                    userCurrencyRate = initialOrder.CurrencyRate;
                }
                ...
                if (!processPaymentRequest.IsRecurringPayment)
				{
					userLanguage = _languageService.GetLanguageById(user.GetAttribute<int>(
						SystemUserAttributeNames.LanguageId, processPaymentRequest.SiteId));
				}
				else
				{
					userLanguage = _languageService.GetLanguageById(initialOrder.UserLanguageId);
				}
                if (userLanguage == null || !userLanguage.Published)
                    userLanguage = _workContext.WorkingLanguage;

                //check whether user is guest
                if (user.IsGuest() && !_orderSettings.AnonymousCheckoutAllowed)
                    throw new WorkException("Anonymous checkout is not allowed");

				var siteId = _siteContext.CurrentSite.Id;
                
                //load and validate user shopping cart
                IList<OrganizedShoppingCartItem> cart = null;
                if (!processPaymentRequest.IsRecurringPayment)
                {
                    //load shopping cart
					cart = user.GetCartItems(ShoppingCartType.ShoppingCart, processPaymentRequest.SiteId);

                    if (cart.Count == 0)
                        throw new WorkException("Cart is empty");

                    //validate the entire shopping cart
					var warnings = _shoppingCartService.GetShoppingCartWarnings(cart,
						user.GetAttribute<string>(SystemUserAttributeNames.CheckoutAttributes), true);
                    if (warnings.Count > 0)
                    {
                        ...
                        throw new WorkException(warningsSb.ToString());
                    }

                    //validate individual cart items
                    foreach (var sci in cart)
                    {
                        var sciWarnings = _shoppingCartService.GetShoppingCartItemWarnings(user, sci.Item.ShoppingCartType,
							sci.Item.Product, processPaymentRequest.SiteId, sci.Item.AttributesXml,
                            sci.Item.UserEnteredPrice, sci.Item.Quantity, false, childItems: sci.ChildItems);
                ...
                if (!processPaymentRequest.IsRecurringPayment)
                {
                    ...
                    {
                        decimal minOrderSubtotalAmount = _currencyService.ConvertFromPrimarySiteCurrency(_orderSettings.MinOrderSubtotalAmount, _workContext.WorkingCurrency);
                        throw new WorkException(string.Format(_localizationService.GetResource("Checkout.MinOrderSubtotalAmount"), _priceFormatter.FormatPrice(minOrderSubtotalAmount, true, false)));
                    }
                    bool minOrderTotalAmountOk = ValidateMinOrderTotalAmount(cart);
                    if (!minOrderTotalAmountOk)
                    {
                        decimal minOrderTotalAmount = _currencyService.ConvertFromPrimarySiteCurrency(_orderSettings.MinOrderTotalAmount, _workContext.WorkingCurrency);
                        throw new WorkException(string.Format(_localizationService.GetResource("Checkout.MinOrderTotalAmount"), _priceFormatter.FormatPrice(minOrderTotalAmount, true, false)));
                    }
                }
                
                //tax display type
                var userTaxDisplayType = TaxDisplayType.IncludingTax;
                if (!processPaymentRequest.IsRecurringPayment)
                {
					userTaxDisplayType = _workContext.GetTaxDisplayTypeFor(user, processPaymentRequest.SiteId);
                }
                else
                {
                    userTaxDisplayType = initialOrder.UserTaxDisplayType;
                }
                ...
                if (!processPaymentRequest.IsRecurringPayment)
                {
					checkoutAttributesXml = user.GetAttribute<string>(SystemUserAttributeNames.CheckoutAttributes);
					checkoutAttributeDescription = _checkoutAttributeFormatter.FormatAttributes(checkoutAttributesXml, user);
                }
                else
                {
                ...
                //applied discount (used to site discount usage history)
                ...
                if (shoppingCartRequiresShipping)
                {
                    if (!processPaymentRequest.IsRecurringPayment)
                    {
						var shippingOption = user.GetAttribute<ShippingOption>(SystemUserAttributeNames.SelectedShippingOption, processPaymentRequest.SiteId);
                ...
                if (!processPaymentRequest.IsRecurringPayment)
                {
                    ...
                    if (!orderShippingTotalInclTax.HasValue || !orderShippingTotalExclTax.HasValue)
                        throw new WorkException("Shipping total couldn't be calculated");
                ...
                if (!processPaymentRequest.IsRecurringPayment)
                {
                    ...
                    paymentAdditionalFeeInclTax = _taxService.GetPaymentMethodAdditionalFee(paymentAdditionalFee, true, user, out paymentAdditionalFeeTaxRate);
                    paymentAdditionalFeeExclTax = _taxService.GetPaymentMethodAdditionalFee(paymentAdditionalFee, false, user);
                }
                else
                {
                ...
                if (!processPaymentRequest.IsRecurringPayment)
                {
					...
					if (_taxSettings.EuVatEnabled && userVatStatus == VatNumberStatus.Valid)
						vatNumber = user.GetAttribute<string>(SystemUserAttributeNames.VatNumber);
                ...
                if (!processPaymentRequest.IsRecurringPayment)
                {
                    ...
                    if (!orderTotal.HasValue)
                        throw new WorkException("Order total couldn't be calculated");
				...
				if (!preProcessPaymentResult.Success)
				{
					...
					throw new WorkException("Error while pre-processing the payment");
				}
				...
				if (!processPaymentRequest.IsRecurringPayment)
				{
					if (user.BillingAddress == null)
						throw new WorkException("Billing address is not provided");

					if (!user.BillingAddress.Email.IsEmail())
						throw new WorkException("Email is not valid");

					billingAddress = (Address)user.BillingAddress.Clone();
				}
				else
				{
					if (initialOrder.BillingAddress == null)
						throw new WorkException("Billing address is not available");
				...
				if (billingAddress.Country != null && !billingAddress.Country.AllowsBilling)
					throw new WorkException(string.Format("Country '{0}' is not allowed for billing", billingAddress.Country.Name));
				...
				if (shoppingCartRequiresShipping)
				{
					if (!processPaymentRequest.IsRecurringPayment)
					{
						if (user.ShippingAddress == null)
							throw new WorkException("Shipping address is not provided");

						if (!user.ShippingAddress.Email.IsEmail())
							throw new WorkException("Email is not valid");
					...
					else
					{
						if (initialOrder.ShippingAddress == null)
							throw new WorkException("Shipping address is not available");
					...
					if (shippingAddress.Country != null && !shippingAddress.Country.AllowsShipping)
						throw new WorkException(string.Format("Country '{0}' is not allowed for shipping", shippingAddress.Country.Name));
				}
				...
				if (!skipPaymentWorkflow)
				{
					...
					if (!paymentMethod.IsPaymentMethodActive(_paymentSettings))
						throw new WorkException("Payment method is not active");
				}
				else
				{
				...
				if (!processPaymentRequest.IsRecurringPayment)
				{
					...
					if (isRecurringShoppingCart)
					{
						...
						if (!string.IsNullOrEmpty(recurringCyclesError))
							throw new WorkException(recurringCyclesError);
                ...
                if (!skipPaymentWorkflow)
                {
                        ...
                        if (isRecurringShoppingCart)
                        {
                            ...
                            switch (recurringPaymentType)
                            {
                                case RecurringPaymentType.NotSupported:
                                    throw new WorkException("Recurring payments are not supported by selected payment method");
                                case RecurringPaymentType.Manual:
                                case RecurringPaymentType.Automatic:
                                    processPaymentResult = _paymentService.ProcessRecurringPayment(processPaymentRequest);
                                    break;
                                default:
                                    throw new WorkException("Not supported recurring payment type");
                            }
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        {
                            throw new WorkException("No recurring products");
                        }
                    }
                }
                else
                ...
                if (processPaymentResult == null)
                    throw new WorkException("processPaymentResult is not available");
                ...
                if (processPaymentResult.Success)
                {
                    ...
                    //using (var scope = new System.Transactions.TransactionScope())
                    {
                        ...
                        var order = new Order()
                        {
							SiteId = processPaymentRequest.SiteId,
                            OrderGuid = processPaymentRequest.OrderGuid,
                            UserId = user.Id,
                            UserLanguageId = userLanguage.Id,
                            UserTaxDisplayType = userTaxDisplayType,
							...
							CreatedOnUtc = utcNow,
							UpdatedOnUtc = utcNow,
                            UserOrderComment = extraData.ContainsKey("UserComment") ? extraData["UserComment"] : ""
                        };
                        ...
                        if (!processPaymentRequest.IsRecurringPayment)
                        {
                            ...
                            foreach (var sc in cart)
                            {
								...
								decimal scUnitPriceInclTax = _taxService.GetProductPrice(sc.Item.Product, scUnitPrice, true, user, out unitPriceTaxRate);
                                decimal scUnitPriceExclTax = _taxService.GetProductPrice(sc.Item.Product, scUnitPrice, false, user, out taxRate);
                                decimal scSubTotalInclTax = _taxService.GetProductPrice(sc.Item.Product, scSubTotal, true, user, out taxRate);
                                decimal scSubTotalExclTax = _taxService.GetProductPrice(sc.Item.Product, scSubTotal, false, user, out taxRate);

                                //discounts
                                Discount scDiscount = null;
                                decimal discountAmount = _priceCalculationService.GetDiscountAmount(sc, out scDiscount);
                                decimal discountAmountInclTax = _taxService.GetProductPrice(sc.Item.Product, discountAmount, true, user, out taxRate);
                                decimal discountAmountExclTax = _taxService.GetProductPrice(sc.Item.Product, discountAmount, false, user, out taxRate);
                                if (scDiscount != null && !appliedDiscounts.ContainsDiscount(scDiscount))
                                    appliedDiscounts.Add(scDiscount);

                                //attributes
                                string attributeDescription = _productAttributeFormatter.FormatAttributes(sc.Item.Product, sc.Item.AttributesXml, user);
								...
								if (sc.Item.Product.ProductType == ProductType.BundledProduct && sc.ChildItems != null)
								{
									...
									foreach (var childItem in sc.ChildItems)
									{
										...
										string attributesInfo = _productAttributeFormatter.FormatAttributes(childItem.Item.Product, childItem.Item.AttributesXml, order.User,
                        ...
                        {
                            user.AddRewardPointsHistoryEntry(-redeemedRewardPoints,
                                string.Format(_localizationService.GetResource("RewardPoints.Message.RedeemedForOrder", order.UserLanguageId), order.GetOrderNumber()),
                                order,
                        ...
                        int orderPlacedSiteOwnerNotificationQueuedEmailId = _workflowMessageService.SendOrderPlacedSiteOwnerNotification(order, _localizationSettings.DefaultAdminLanguageId);
                        if (orderPlacedSiteOwnerNotificationQueuedEmailId > 0)
                        {
                            order.OrderNotes.Add(new OrderNote()
                            {
                                Note = string.Format(T("MerchantEmailQueued"), orderPlacedSiteOwnerNotificationQueuedEmailId),
                                DisplayToUser = false,
								CreatedOnUtc = utcNow
                            });
                            _orderService.UpdateOrder(order);
                        ...
                        int orderPlacedUserNotificationQueuedEmailId = _workflowMessageService.SendOrderPlacedUserNotification(order, order.UserLanguageId);
                        if (orderPlacedUserNotificationQueuedEmailId > 0)
                        {
                            order.OrderNotes.Add(new OrderNote()
                            {
                                Note = string.Format(T("UserEmailQueued"), orderPlacedUserNotificationQueuedEmailId),
                                DisplayToUser = false,
								CreatedOnUtc = utcNow
                            });
                            _orderService.UpdateOrder(order);
                        ...
                        //reset checkout data
                        if (!processPaymentRequest.IsRecurringPayment)
							_userService.ResetCheckoutData(user, processPaymentRequest.SiteId, clearCouponCodes: true, clearCheckoutAttributes: true);
						...
						{
							_genericAttributeService.SaveAttribute<object>(order, customProperty.Key, customProperty.Value.Value, order.SiteId);
						}
                        ...
                        if (!processPaymentRequest.IsRecurringPayment)
                        {
                            _userActivityService.InsertActivity(
                                "PublicSite.PlaceOrder",
                                _localizationService.GetResource("ActivityLog.PublicSite.PlaceOrder"),
        ...
        public virtual void ProcessNextRecurringPayment(RecurringPayment recurringPayment)
        {
            ...
            try
            {
                if (!recurringPayment.IsActive)
                    throw new WorkException("Recurring payment is not active");

                var initialOrder = recurringPayment.InitialOrder;
                if (initialOrder == null)
                    throw new WorkException("Initial order could not be loaded");

                var user = initialOrder.User;
                if (user == null)
                    throw new WorkException("User could not be loaded");

                var nextPaymentDate = recurringPayment.NextPaymentDate;
                if (!nextPaymentDate.HasValue)
                    throw new WorkException("Next payment date could not be calculated");

                //payment info
                var paymentInfo = new ProcessPaymentRequest()
                {
					SiteId = initialOrder.SiteId,
                    UserId = user.Id,
                ...
                if (result.Success)
                {
                    if (result.PlacedOrder == null)
                        throw new WorkException("Placed order could not be loaded");
            ...
            try
            {
                ...
                if (result.Success)
                {
                    ...
                    _workflowMessageService
                        .SendRecurringPaymentCancelledSiteOwnerNotification(recurringPayment, 
        ...
        public virtual void Ship(Shipment shipment, bool notifyUser)
        {
            ...
            {
                //notify user
                int queuedEmailId = _workflowMessageService.SendShipmentSentUserNotification(shipment, order.UserLanguageId);
                if (queuedEmailId > 0)
                {
                    order.OrderNotes.Add(new OrderNote()
                    {
                        Note = string.Format(T("UserShippedEmailQueued"), queuedEmailId),
                        DisplayToUser = false,
                        CreatedOnUtc = DateTime.UtcNow
                    });
                    _orderService.UpdateOrder(order);
        ...
        public virtual void Deliver(Shipment shipment, bool notifyUser)
        {
            ...
            if (notifyUser)
            {
                ...
                int queuedEmailId = _workflowMessageService.SendShipmentDeliveredUserNotification(shipment, order.UserLanguageId);
                if (queuedEmailId > 0)
                {
                    order.OrderNotes.Add(new OrderNote()
                    {
                        Note = string.Format(T("UserDeliveredEmailQueued"), queuedEmailId),
                        DisplayToUser = false,
                        CreatedOnUtc = DateTime.UtcNow
                    });
                    _orderService.UpdateOrder(order);
        ...
        public virtual void CancelOrder(Order order, bool notifyUser)
        {
            ...
            if (!CanCancelOrder(order))
                throw new WorkException("Cannot do cancel for order.");
		...
		public virtual void AutoUpdateOrderDetails(AutoUpdateOrderItemContext context)
		{
			...
			context.RewardPointsOld = context.RewardPointsNew = oi.Order.User.GetRewardPointsBalance();
			...
			if (context.UpdateRewardPoints && context.QuantityDelta < 0)
			{
				...
				context.RewardPointsNew = oi.Order.User.GetRewardPointsBalance();
			}
		}
		...
		public virtual void CompleteOrder(Order order)
		{
            if (!CanCompleteOrder(order))
                throw new WorkException("You can't mark this order as completed");
        ...
        public virtual IList<string> Capture(Order order)
        {
            ...
            if (!CanCapture(order))
                throw new WorkException("Cannot do capture for order.");
        ...
        public virtual void MarkOrderAsPaid(Order order)
        {
            ...
            if (!CanMarkOrderAsPaid(order))
                throw new WorkException("You can't mark this order as paid");
        ...
        public virtual IList<string> Refund(Order order)
        {
            ...
            if (!CanRefund(order))
                throw new WorkException("Cannot do refund for order.");
        ...
        public virtual void RefundOffline(Order order)
        {
            ...
            if (!CanRefundOffline(order))
                throw new WorkException("You can't refund this order");
        ...
        public virtual IList<string> PartiallyRefund(Order order, decimal amountToRefund)
        {
            ...
            if (!CanPartiallyRefund(order, amountToRefund))
                throw new WorkException("Cannot do partial refund for order.");
        ...
        public virtual void PartiallyRefundOffline(Order order, decimal amountToRefund)
        {
            ...
            if (!CanPartiallyRefundOffline(order, amountToRefund))
                throw new WorkException("You can't partially refund (offline) this order");
        ...
        public virtual IList<string> Void(Order order)
        {
            ...
            if (!CanVoid(order))
                throw new WorkException("Cannot do void for order.");
        ...
        public virtual void VoidOffline(Order order)
        {
            ...
            if (!CanVoidOffline(order))
                throw new WorkException("You can't void this order");
        ...
        public virtual void ReOrder(Order order)
        {
            ...
            foreach (var orderItem in order.OrderItems)
            {
				...
				addToCartContext.Warnings = _shoppingCartService.AddToCart(order.User, orderItem.Product, ShoppingCartType.ShoppingCart, order.SiteId,
				...
				if (isBundle && orderItem.BundleData.HasValue() && addToCartContext.Warnings.Count == 0)
				{
					foreach (var bundleData in orderItem.GetBundleData())
					{
						...
						addToCartContext.Warnings = _shoppingCartService.AddToCart(order.User, bundleItem.Product, ShoppingCartType.ShoppingCart, order.SiteId,
