    ...
    public static class ShoppingCartExtensions
    {
            ...
            out int cycleLength, out RecurringProductCyclePeriod cyclePeriod, out int totalCycles)
        {
            ...
            foreach (var sci in shoppingCart)
            {
                ...
                if (product == null)
                {
                    throw new WorkException(string.Format("Product (Id={0}) cannot be loaded", sci.Item.ProductId));
                }
		...
		public static IEnumerable<ShoppingCartItem> Filter(this IEnumerable<ShoppingCartItem> shoppingCart, ShoppingCartType type, int? siteId = null)
		{
			var enumerable = shoppingCart.Where(x => x.ShoppingCartType == type);

			if (siteId.HasValue)
				enumerable = enumerable.Where(x => x.SiteId == siteId.Value);
