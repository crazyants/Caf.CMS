...
namespace CAF.WebSite.Application.Services.Messages
{
    ...
    public class NewsLetterSubscriptionService : INewsLetterSubscriptionService
    {
            ...
            try
            {
                using (var reader = new StreamReader(stream))
                {
                    ...
                    while (!reader.EndOfStream)
                    {
                        ...
                        {
                            throw new WorkException("Wrong file format (expected comma separated entries 'Email' and optionally 'IsActive')");
                        }
                        ...
                        if (siteId == 0)
                        {
                            siteId = _siteService.GetAllSites().First().Id;
                        }
        ...
        public void InsertNewsLetterSubscription(NewsLetterSubscription newsLetterSubscription, bool publishSubscriptionEvents = true)
        {
            ...
            if (newsLetterSubscription.SiteId == 0)
            {
                throw new WorkException("News letter subscription must be assigned to a valid site.");
            }
        ...
        /// <returns>NewsLetter subscription</returns>
        public virtual NewsLetterSubscription GetNewsLetterSubscriptionByEmail(string email, int siteId = 0)
        {
            if (!email.IsEmail())
                return null;
        ...
        public virtual IPagedList<NewsLetterSubscription> GetAllNewsLetterSubscriptions(string email, int pageIndex, int pageSize, bool showHidden = false, int siteId = 0)
        {
            var query = _subscriptionRepository.Table;
            ...
            query = query.OrderBy(nls => nls.Email).ThenBy(x => x.SiteId);
