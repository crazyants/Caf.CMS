...
namespace CAF.WebSite.Application.Services.Messages
{
 ...
 IWebHelper webHelper,
        ...
        //            string downloadUrl = string.Format("{0}download/getdownload/{1}", _webHelper.GetSiteLocation(false), orderItem.OrderItemGuid);
        ...
        //    if (!String.IsNullOrEmpty(order.ExtendedAttributeDescription))
        //    {
        //        sb.AppendLine("<tr><td style=\"text-align:right;\" colspan=\"1\">&nbsp;</td><td colspan=\"3\" style=\"text-align:right\">");
        //        //codehint: sm-edit
        //        //sb.AppendLine(order.ExtendedAttributeDescription);
        //        sb.AppendLine(HtmlUtils.ConvertPlainTextToTable(HtmlUtils.ConvertHtmlToPlainText(order.ExtendedAttributeDescription)));
        //        sb.AppendLine("</td></tr>");
        //    }
        ...
        protected virtual string GetSupplierIdentification()
        {
            ...
            if (!String.IsNullOrEmpty(_siteContext.CurrentSite.Url))
            {
                sb.AppendLine(String.Format("Url: <a href=\"{0}\">{0}</a><br>", _siteContext.CurrentSite.Url));
            }
        ...
        public virtual void AddSiteTokens(IList<Token> tokens, Site site)
        {
        ...
        //    tokens.Add(new Token("Order.UserComment", order.UserOrderComment, true));
        ...
        //    //TODO add a method for getting URL (use routing because it handles all SEO friendly URLs)
        //    tokens.Add(new Token("Order.OrderURLForUser", string.Format("{0}order/details/{1}", _webHelper.GetSiteLocation(false), order.Id), true));
        ...
        //    tokens.Add(new Token("Shipment.URLForUser", string.Format("{0}order/shipmentdetails/{1}", _webHelper.GetSiteLocation(false), shipment.Id), true));
        ...
        //    tokens.Add(new Token("ReturnRequest.UserComment", HtmlUtils.FormatText(returnRequest.UserComments, false, true, false, false, false, false), true));
        ...
        public virtual void AddUserTokens(IList<Token> tokens, User user)
        {
            ...
            //TODO add a method for getting URL (use routing because it handles all SEO friendly URLs)
            string passwordRecoveryUrl = string.Format("{0}user/passwordrecoveryconfirm?token={1}&email={2}", _webHelper.GetSiteLocation(),
                user.GetAttribute<string>(SystemUserAttributeNames.PasswordRecoveryToken), HttpUtility.UrlEncode(user.Email));

            string accountActivationUrl = string.Format("{0}user/activation?token={1}&email={2}", _webHelper.GetSiteLocation(),
                user.GetAttribute<string>(SystemUserAttributeNames.AccountActivationToken), HttpUtility.UrlEncode(user.Email));

            var wishlistUrl = string.Format("{0}wishlist/{1}", _webHelper.GetSiteLocation(), user.UserGuid);
        ...
        //    // TODO: add a method for getting URL (use routing because it handles all SEO friendly URLs)
        //    var productUrl = string.Format("{0}{1}", _webHelper.GetSiteLocation(false), product.GetSeName());
