...
//超链接
var ulwz=1000-20-$('.BotSide h3.linkLJ').width();
	    $('.BotSide ul').css('width',ulwz);
	    $('.WBtxt').css('width',ulwz);
//搜索宽度	
	var SS=$('.sskd').width();
	var GG=$('.sskd').height();
	var KD=665-SS+'px';
	var GD=GG-2+'px';
	$('.navsearch_input input').css('width',KD);
	$('.navsearch_input input').css('height',GD);
//等高
    metHeight($(".DG-con"));
	metHeight($(".bottom-DG"));
...
if(module==10001){
	...
	//超链接
		var ulwz=926-20-$('.BotSideIN h3.linkLJ').width();
	    $('.BotSideIN ul').css('width',ulwz);
